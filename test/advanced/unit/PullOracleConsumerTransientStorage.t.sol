...
import {PullOracleConsumerTransientStorageMock} from "test/advanced/mocks/PullOracleConsumerTransientStorageMock.sol";
...
/*
 * Test suite for PullOracleConsumerTransientStorage
 */
contract PullOracleConsumerTransientStorageTest is BaseTest {
    using LowLevelReverter for address;

    PullOracleConsumerTransientStorageMock internal mock;
    ...
    function setUp() public {
        ...
        mock = new PullOracleConsumerTransientStorageMock(INIT_COUNT, INIT_DELAY, INIT_DRIFT, signers);
    }
    ...
    function test_Constructor_EmitsSignerEvents() public {
        ...
        emit IPullOracleConsumerStorage.SignerStatusUpdated(PRIMARY_SIGNER, true);
        vm.expectEmit(true, false, false, true);
        emit IPullOracleConsumerStorage.SignerStatusUpdated(SECONDARY_SIGNER, true);

        new PullOracleConsumerTransientStorageMock(INIT_COUNT, INIT_DELAY, INIT_DRIFT, signers);
    }
    ...
    function test_Constructor_ZeroDrift_Allowed() public {
        ...
        PullOracleConsumerTransientStorageMock m = new PullOracleConsumerTransientStorageMock(
            INIT_COUNT,
            INIT_DELAY,
            0,
            signers
    ...
    function test_Revert_Constructor_ZeroMaxPackageCount() public {
        ...
        new PullOracleConsumerTransientStorageMock(0, INIT_DELAY, INIT_DRIFT, new address[](0));
    }
    ...
    function test_Revert_Constructor_ZeroMaxDelay() public {
        ...
        new PullOracleConsumerTransientStorageMock(INIT_COUNT, 0, INIT_DRIFT, new address[](0));
    }
    ...
    function test_SetMaxPackageCount_Success() public {
        ...
        emit IPullOracleConsumerStorage.MaxPackageCountUpdated(uint256(INIT_COUNT), uint256(newCount));
    ...
    function test_SetMaxDelay_Success() public {
        ...
        vm.expectEmit(false, false, false, true);
        emit IPullOracleConsumerStorage.MaxDelayUpdated(uint256(INIT_DELAY), uint256(newDelay));
    ...
    function test_SetMaxFutureDrift_Success() public {
        ...
        vm.expectEmit(false, false, false, true);
        emit IPullOracleConsumerStorage.MaxFutureDriftUpdated(uint256(INIT_DRIFT), uint256(newDrift));
    ...
    function test_SetSignerStatus_Add() public {
        vm.expectEmit(true, false, false, true);
        emit IPullOracleConsumerStorage.SignerStatusUpdated(UNAUTHORIZED_SIGNER, true);
    ...
    function test_SetSignerStatus_Remove() public {
        vm.expectEmit(true, false, false, true);
        emit IPullOracleConsumerStorage.SignerStatusUpdated(PRIMARY_SIGNER, false);
    ...
    /* ————————————————————————————————————————————————————————————————————————
                    INTEGRATION TESTS: TRANSIENT BATCH STRICT MODE
    ————————————————————————————————————————————————————————————————————————— */

    /*
     * Verify successful transient batch retrieval with storage-backed authorization
     */
    function test_Integration_ExecuteWithFeedDataBatchTransient_Success() public {
        uint256 count = 3;
        ...
        bytes memory extraData = _buildSignedExtraData(PRIMARY_SIGNER_PK, ids, prices, timestamps);
        bytes memory businessCall = abi.encodeWithSelector(
            mock.executeWithFeedDataBatchTransient.selector,
            requestedIds,
            hex"cafebabe"
        );
    ...
    /*
     * Verify transient batch strict mode reverts when one feed ID is missing
     */
    function test_Integration_ExecuteWithFeedDataBatchTransient_Revert_UnmatchedFeedID() public {
        uint256 count = 2;
    ...
    /*
     * Verify transient batch strict mode reverts on unauthorized signer
     */
    function test_Integration_ExecuteWithFeedDataBatchTransient_Revert_UnauthorizedSigner() public {
        uint256 count = 2;
    ...
    /*
     * Verify transient batch strict mode signer update takes effect
     */
    function test_Integration_ExecuteWithFeedDataBatchTransient_SignerUpdateTakesEffect() public {
        uint256 count = 2;
    ...
    /*
     * Verify transient batch strict mode reverts when one matched feed has expired timestamp
     */
    function test_Integration_ExecuteWithFeedDataBatchTransient_Revert_PartialExpired() public {
        uint256 count = 2;
    ...
    /*
     * Verify transient batch strict mode reverts when one matched feed exceeds future drift tolerance
     */
    function test_Integration_ExecuteWithFeedDataBatchTransient_Revert_PartialFutureDrift() public {
        uint256 count = 2;
    ...
    /*
     * Verify transient batch strict mode config update takes effect for maxDelay
     */
    function test_Integration_ExecuteWithFeedDataBatchTransient_ConfigUpdateTakesEffect_MaxDelay() public {
        uint256 count = 2;
    ...
    /*
     * Verify transient batch strict mode config update takes effect for maxFutureDrift
     */
    function test_Integration_ExecuteWithFeedDataBatchTransient_ConfigUpdateTakesEffect_MaxFutureDrift() public {
        uint256 count = 2;
    ...
    /*
     * Verify transient batch strict mode reverts when payload exceeds maxPackageCount
     */
    function test_Integration_ExecuteWithFeedDataBatchTransient_Revert_ExceedsMaxPackageCount() public {
        uint256 packageCount = uint256(INIT_COUNT) + 1;
    ...
    /*
     * Verify transient batch strict mode config update takes effect for maxPackageCount
     */
    function test_Integration_ExecuteWithFeedDataBatchTransient_ConfigUpdateTakesEffect_MaxPackageCount() public {
        uint256 packageCount = uint256(INIT_COUNT) + 1;
     ...
     * Verify empty feedIds array short-circuits before authentication and returns empty arrays
     */
    function test_Integration_ExecuteWithFeedDataBatchTransient_Success_EmptyFeedIds() public {
        bytes4[] memory emptyIds = new bytes4[](0);
    ...
    /* ————————————————————————————————————————————————————————————————————————
                    INTEGRATION TESTS: TRANSIENT BATCH LENIENT MODE
    ————————————————————————————————————————————————————————————————————————— */

    /*
     * Verify transient batch lenient mode returns zero for unmatched feeds
     */
    function test_Integration_ExecuteWithFeedDataBatchLenientTransient_Success_PartialUnmatched() public {
        uint256 count = 2;
        ...
        bytes memory extraData = _buildSignedExtraData(PRIMARY_SIGNER_PK, ids, prices, timestamps);
        bytes memory businessCall = abi.encodeWithSelector(
            mock.executeWithFeedDataBatchLenientTransient.selector,
            requestedIds,
            hex"cafebabe"
        );
    ...
    /*
     * Verify transient batch lenient mode reverts on unauthorized signer
     */
    function test_Integration_ExecuteWithFeedDataBatchLenientTransient_Revert_UnauthorizedSigner() public {
        uint256 count = 2;
    ...
    /*
     * Verify transient batch lenient mode signer update takes effect
     */
    function test_Integration_ExecuteWithFeedDataBatchLenientTransient_SignerUpdateTakesEffect() public {
        uint256 count = 2;
    ...
    /*
     * Verify transient batch lenient mode still reverts on expired matched feed
     */
    function test_Integration_ExecuteWithFeedDataBatchLenientTransient_Revert_PartialExpired() public {
        uint256 count = 2;
    ...
    /*
     * Verify transient batch lenient mode reverts when one matched feed exceeds future drift tolerance
     */
    function test_Integration_ExecuteWithFeedDataBatchLenientTransient_Revert_PartialFutureDrift() public {
        uint256 count = 2;
    ...
    /*
     * Verify transient batch lenient mode config update takes effect for maxDelay
     */
    function test_Integration_ExecuteWithFeedDataBatchLenientTransient_ConfigUpdateTakesEffect_MaxDelay() public {
        uint256 count = 2;
    ...
    /*
     * Verify transient batch lenient mode config update takes effect for maxFutureDrift
     */
    function test_Integration_ExecuteWithFeedDataBatchLenientTransient_ConfigUpdateTakesEffect_MaxFutureDrift() public {
        uint256 count = 2;
    ...
    /*
     * Verify transient batch lenient mode reverts when payload exceeds maxPackageCount
     */
    function test_Integration_ExecuteWithFeedDataBatchLenientTransient_Revert_ExceedsMaxPackageCount() public {
        uint256 packageCount = uint256(INIT_COUNT) + 1;
    ...
    /*
     * Verify transient batch lenient mode config update takes effect for maxPackageCount
     */
    function test_Integration_ExecuteWithFeedDataBatchLenientTransient_ConfigUpdateTakesEffect_MaxPackageCount()
        public
    {
        uint256 packageCount = uint256(INIT_COUNT) + 1;
     ...
     * Verify empty feedIds array short-circuits before authentication and returns empty arrays
     */
    function test_Integration_ExecuteWithFeedDataBatchLenientTransient_Success_EmptyFeedIds() public {
        bytes4[] memory emptyIds = new bytes4[](0);
